...
@section('content')
    <div class="card">
        <div class="card-header header-elements-inline">
            <h2 class="card-title"><b>@lang('app.shelves')</b></h2>
            ...
            <div class="datatable-header">
              @can('add_shelf')
                <div class="dt-buttons">
                    <button class=" rounded-round dt-button buttons-pdf buttons-html5 btn bg-primary-400" tabindex="0" aria-controls="DataTables_Table_1" type="button"><span><a href="{{route('shelf.create')}}" style="color: #ffffff">@lang('app.add_shelf')</a> <i class="icon-googleplus5"></i></span></button>
                </div>
                @endcan
            </div>
            ...
            <div class="datatable-scroll">
                <table class="table datatable-reorder dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                    ...
                    <tr role="row" class="table-active">
                        ...
                        <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" data-column-index="1" aria-label="Last Name: activate to sort column ascending">@lang('app.shelf_name')</th>
                        <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" data-column-index="1" aria-label="Last Name: activate to sort column ascending">@lang('app.status')</th>
                        <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" data-column-index="1" aria-label="Last Name: activate to sort column ascending">@lang('app.capacity')</th>
                    ...
                    <tbody>
                    @foreach($shelf as $shel)

                        <tr role="row" class="odd">
                            <td class="text-center">{{++$i}}</td>
                            <td class="text-center">{{$shel->name}}</td>
                            <td class="text-center">{{$shel->status_name ?? "--"}}</td>
                            <td class="text-center">{{$shel->capacity}}</td>
                            <td class="text-center">{{$shel->shelf_number}}</td>
                            <td hidden></td>
                            <td class="text-center" >
                                <div class="list-icons">
                                    @can('edit_shelf')
                                    <a href="{{route('shelf.edit',$shel->id)}}" class="list-icons-item">
                                    ...
                                    @can('delete_shelf')
                                       ...
                                       data-target="#delete_item_modal">
                                        ...
                                        @can('view_shelf')
                                        <a href="{{route('shelf.show',$shel->id)}}" class="list-icons-item">
    ...
    <script>
        function delete_shelf(id, title) {
            $('#item_id').val(id);
            var url = "{{url('shelf')}}/" + id;
